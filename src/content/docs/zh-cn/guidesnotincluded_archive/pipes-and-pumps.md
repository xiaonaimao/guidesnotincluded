...
全文以一套厕所加自供电制氧模块的系统为例，说明控制液体流动可以怎么做、以及为什么值得学会这么做。
...
这个机制还能用在给模块供水之类的场景：你可能希望优先用某个来源的水，比如厕所的溢流水，其他时候用另一个来源。（本节末尾有这个例子的配图。）
...
1. 用作厕所净化系统的溢流机构。
2. 用作 自供电制氧模块中多余氢气的溢流机构。

## 示例系统：厕所、模块与水源

最后用一个例子收尾，展示液体管道桥如何整合多个系统。这个例子（的简化版）几乎出现在我的每一次游玩里：把厕所的多余水送到模块。
...
流量优先级。厕所污水净化系统的多余水被送往模块。（这里的管道还没连完——最终版看最后一张图。）

上图中，中间是一套污水净化系统。（它的工作原理和搭建方法在指南另一章：见[《回收厕所水》](recycling-toilet-water.md)。）

刚净化的水首先被送回厕所回路。这靠一个液体管道桥实现——画面最右侧中间那个——它告诉水：下面的管道有空位就下去。

因为厕所产出的液体比消耗的多，厕所加净化系统会富余出水。当从液体管道桥向下的管道灌满后，多余的水就会被送上去给模块。

（模块是一套产氧装置，指南后面会讲：见[《氧气：自供电制氧模块》](to-know-the-spom-is-to-love-the-spom.md)。）

还没完。照上图搭的话，只有厕所恰好富余时模块才能零星喝到水。

我们要保证模块随时有水喝。办法是从另一个水源再接一条管道过来。（水不够用的话，见[《获取（更多）水》](getting-more-water.md)。）

![ONI_Guide_PrioritizingFlow2.png](../../guidesnotincluded_archive/assets/ONI_Guide_PrioritizingFlow2-c9ea6cfdf3.avif)

快好了。水源接到模块的这个液体管道桥意味着：只有厕所那条管道空了，它才会用水源的水。但它会同时把水既泵给模块（好事），又泵回厕所（坏事）。
...
上图的用法保证了模块优先喝厕所的富余水，只在没有富余时才动用水源。

（其实厕所的富余水没那么多，模块的耗水量又不小。所以严格说不加这个桥也行。就算模块把厕所富余水和水源水平均着喝，也不太可能堵死厕所。但厕所一旦堵了就要出事故。所以加一道额外的保险——哪怕多半用不上——总是好的。）

还剩最后一个问题：上图中，水源的水会被同时泵给模块和厕所。后者会堵死厕所并引发事故。

我们要确保水源的水只给模块、绝不进厕所。办法是再给设计加一个液体管道桥，挡住从水源流向厕所的水。

![ONI_Guide_PrioritizingFlow5.png](../../guidesnotincluded_archive/assets/ONI_Guide_PrioritizingFlow5-8ca633577f.avif)

厕所与模块。这是我几乎每局都会用的（简化版）系统。厕所（以及淋浴间、污水倾倒站等）的富余水被送往模块。模块同时还需要一个额外水源。液体管道桥在各处控制着水的流向。
