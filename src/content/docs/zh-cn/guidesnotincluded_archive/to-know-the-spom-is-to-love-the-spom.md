...
每个《缺氧》玩家的生涯里都有几个关键节点——那些永远改变你玩法瞬间。建造你的第一座自供电制氧模块（英文社区缩写 SPOM，Self-Powering Oxygen Machine——wiki.gg 用 Machine，QuQuasar 原帖作 Module，两说并存），多半就是其中之一。

该模块把水变成氧气——并且在此过程中产生足够的电力给自己供电。建成之后，你只需供水，它就会永远产氧、永远自给自足地发电。

这个名字最早由 QuQuasar 于 2018 年在 Klei 论坛的两篇帖子中提出（[模块第一版](https://forums.kleientertainment.com/forums/topic/87501-self-powering-oxygen-module-production-and-cooling/)、[模块第二版](https://forums.kleientertainment.com/forums/topic/87548-self-powering-oxygen-module-mkii-production-and-cooling/)）。游戏机制后来变了，那些原始设计已无法按预期工作，但模块精神不死。
...
《缺氧》里有好几种产氧机器。你至少应该见过氧气扩散器——吃藻类产氧气。模块用的则是一种叫做电解器的机器。
...
要实现自供电，模块还需要第二台机器：氢气发电机。氢气发电机消耗氢气、产出电力（和热量）。

气体重量各不相同；氢气比氧气轻。电解器产生的氢气会一路升到氧气上方。模块正是利用这个气体机制，把气泵一上一下布置——高的抽氢气，低的抽氧气。

模块的基本思路就是把电解器产出的两种气体分开：

- 氧气泵进你的基地
- 氢气泵给氢气发电机

模块的电解器产出的氢气，给模块自己供电绰绰有余——甚至还有富余。
...
电解器产出的是 +70C（或更热）的氧气。好消息是，你的模块要把基地实际加热到那个温度得花非常久。坏消息是，它依然在持续不断地给你的基地添热。
...
（殖民地小的话，我通常两个一起用：我的冷却回路既穿过基地、也穿过模块，两头一起凉。）

建模块本身不难，难的是冷却。更准确地说，是搞到冷却建筑所需的材料——具体来说，[塑料](getting-oil-petroleum-and-plastic.md)和/或[钢](getting-steel.md)。

下面是三种冷却方案，按难度递增排列。

1. 一个简单但临时的冷却办法：让模块出来的氧气气体管道穿过冰雪群系。管道里的氧气会被冷却。（用导热管道可以改善氧气与冰雪群系之间的[温度传递](heat-transfer-basics.md)。）
...
再重复一遍前面的要点：如果你的基地是"正常"温度，模块把它显著加热需要很长时间。就连模块内部升到 +75C——非黄金机器开始受伤的温度——也要很久。

我曾把模块冷却拖后几百个周期，从未见过它过热。所以，冷却早晚要做，但不必在点火模块的当天就备好。（几乎所有复制人都会告诉你：藻类见底的时候，热的氧气也比没有强。）
...
模块可以建各种尺寸。本站有两座模块的搭建指南：[小型（氧气 1 千克/秒）](spom-1kg-s.md)和[大型（氧气 3 千克/秒）](spom-3kg-s.md)。选哪个主要取决于你想养多少复制人。
...
小型模块产氧 1 千克/秒。理论上够 10 个标准复制人呼吸。不过产氧最好留点余量——你要有足够的富余去灌满你的基地和所有气压服。

一些火箭和太空探索相关的东西也需要氧气。但如果这些现在让你头大，可以完全不管——以后需要时再建一座模块就是。

所以粗略的经验法则是：

- 复制人少于 10 个（标准耗氧）的话，小型模块就够了。（但为了留安全余量，我说 8 个或更少。）
- 要养 10 个或更多复制人（标准耗氧），你要么上更大的模块，要么多建几座小的。
...
模块需要稳定供水。所以一个选址思路是靠近某种水喷口。如果你要用热水，这一点尤其重要。模块离得近，热水管道就不用铺老远。（不管怎样，记得用隔热液体管道减少热扩散。）

冷却

你打算怎么冷却氧气？（如果不冷却氧气，那就是冷却基地。）如果你要用反熵消热器冷却氧气，把模块建在 AETN 附近能把热氧气的输送距离压到最短。
...
这座模块用了两台电解器。气泵布置在略有不同的高度——上面的泵抽氢气，下面两台抽氧气。
...
如果没有金子，可以用钢造机器。或者干脆让冷却管穿过模块——那样机器随便什么材料都行。（连隔热砖都省了。）

接下来是楼上——电力相关的东西。基本思路：把氢气变成模块的电力。这一层的搭法有好几种，这是一种标准做法。
...
除此之外，模块还有连接到气泵的气压传感器（上面图里没有）。它们维持氧气与氢气的平衡，并确保模块的气体储量不会过低。最后，还有一大堆电线、管道等等（同样没出现在上面的图里）。
...
模块产出的氢气比氢气发电机需要的多。所以你需要一个溢流机构来处理多余的氢气。

这些年里，我的模块每次出问题，基本都是因为我先建了模块，想着"等会儿"再去建溢流机构——"等手头这几件事干完"。
...
模块气路示例。氢气先经过一个气体分离器（设为氢气）。然后到达一个气体管道桥——气体优先向左走，去储气库和氢气发电机。满了之后，多余气体分流到气体装罐器和右侧的溢流管道。

我所有的模块都装了常驻氢气气体分离器。比我胆大的人会省掉它。如果你的模块永远不出岔子，气体分离器纯属浪费电。我倾向于认定某天总会出岔子（比如氢气堵管或供水中断），所以装上分离器当保险。

（上例中我没有加冷却。如果这是实际游玩截图，我会让导热液体管道带着冷却穿过模块。因为加了冷却，我的模块用普通砖而不是隔热砖建造。）
...
第一次点火模块时，气体需要一小会儿才能稳定（氧气在下、氢气在上）。在此之前，你的氧气体管道道和氢气体管道道道里都会是混合气体。

想查看模块里的气体混合情况，可以用材料概览（右上角的方形图标，或按 F4）。目标是让顶部区域——特别是（小型模块上）氢气泵下方、两扇门之间的格子——只有氢气。

氢气发电机吃了氢气以外的任何东西都会受损。这个问题有几种解法，举例：

- 简单解法：把初始气体直接排进基地，等气体稳定后再把模块接上氢气发电机。
- 如果不想让氢气进基地，可以先把初始气体泵进储气库，日后再处理，比如排进太空。
- 另一个选项——也是我的习惯——是在氢气体管道道道上加一个气体分离器。模块产的电足够给分离器供电，氢气照样有富余。
...
如果你在想：氧气能不能也加一个气体分离器？同样用模块供电？——可以。我自己不这么干，但根据（有限的）测试完全可行。只是模块产氧几乎是产氢的十倍，氧气分离器的运行频率会远高于氢气分离器，耗电也多得多。

这种设计有时耗电会超过一台氢气发电机的产出。所以想上双分离器的话，加第二台氢气发电机来满足增长的需求。

配氧气和氢气分离器的模块。

根据有限测试，这套建筑看起来可行。图中储气库换成了第二台氢气发电机以满足电力需求。两台分离器都由模块供电，氢气甚至还有一点点富余。

![ONI-SPOM-bootstrapping.png](../../guidesnotincluded_archive/assets/ONI-SPOM-bootstrapping-2fd4206d3c.avif)

模块起步。临时电力装置帮你把模块转起来。也可以直接接到你的高功率电线上。

另外，如果你的目标是保护气压服存放柜不受损，可以省很多电：把一个气体分离器放在只过滤"进气压服存放柜的氧气"的位置。

一个做法是在模块旁分出一条"保证是氧气"的管道和一条"大概是氧气"的管道，前者穿过气体分离器，专供那些吃了氢气会坏的东西。我有时就这么干。（再说一次，比我胆大的人信任他们的模块，根本不操心这些。）

接下来是给模块通电。

模块转起来之后就能自供电，但启动需要帮一把。你可以把模块接进电网，或者扔几台仓鼠轮加一块电池接上去，等模块运转起来再断开。

（哦对了，记得把模块智能蓄电池的充电下限调到 0 以上。这样在氢气发电机接管之前，电池不会被完全放空。）

基本就是这些——开工吧！等模块基础用熟了，就开始按自己的风格和需求改造设计吧。

对我来说，任何新建筑最难的不是动手建，而是鼓起勇气开始。下面是两座模块的详细图片和各种概览的链接，助你踏上无限氧气之路。

- [小型模块](spom-1kg-s.md)（1 千克/秒氧气）
- [大型模块](spom-3kg-s.md)（3 千克/秒氧气）

上面链接的页面里还有 Francis John 讲解这两个建筑的油管视频链接。

最后，给那些还需要一个理由来拖延建造第一座模块的人：这里有一些"野外模块"的照片，看看别人家的模块，顺便为自己的开工做准备。（你可以的！）

## 野外的模块：示例、水喷口与反熵消热器

有一次游玩，我恰好抽到水喷口和反熵消热器挨在一起。于是我把模块建在了它们周围。

这座模块是大型的，也就是社区所称的完整 Rodriguez（4 台电解器的规模）——Rodriguez（社区以设计者 Nicolás Rodriguez 命名的电解制氧布局：利用氢氧密度差上下分层实现无过滤分离，省去气体过滤器的电耗；变体规模按电解器台数划分））。（带一些小改动。）

AETN 冷却能力不太够——氧气出来是 34-35C。（实际基地里还有额外的冷却回路。）

水泵外部供电，两台气体分离器（一台氢气一台氧气）由模块供电。
...
- 这座模块产出三管氧气。（是最下面一排六台气泵——每条氧气体管道两台泵。）
...
建这套时水喷口还在休眠，所以加了一条注水管，等喷口醒过来。那个液体管道桥是为了让模块尽可能用水喷口的水（而不是外面的水管）。
...
本节以 QuQuasar 2018 年模块MKII 帖子中的一段话作结：

"来吧：这是我第二次尝试制作自供氧模块（Self-Powering Oxygen Module），简称 '模块'。
